...
# Constants
API_URL = "https://api-preprod.phonepe.com/apis/pg-sandbox/pg/v1"
...
def generate_unique_id(prefix):
    return prefix + str(uuid.uuid4()).replace('-', '')[:32]
...
def generate_payload(appointment_transaction_id, amount, userID, merchantTransactionID, webhook_url):
    combined = f"{appointment_transaction_id}:{merchantTransactionID}"
    encoded_tx_id = string_to_base64(combined)
    return {
        "merchantId": MERCHANT_ID,
        "merchantTransactionId": merchantTransactionID,
        "merchantUserId": userID,
        "amount": amount * 100,
        # "redirectUrl": f"{settings.FRONTEND_URL}redirect/{encoded_tx_id}",
        "redirectUrl": f"{settings.FRONTEND_URL}patients/thankyou?id={encoded_tx_id}",
...
def generate_phonepe_payload(appointment_transaction_id, amount: int):
    userID = generate_unique_id("user-")
    merchantTransactionID = generate_merchant_transaction_id()

    payload = generate_payload(appointment_transaction_id, amount, userID, merchantTransactionID, WEBHOOK_URL)
